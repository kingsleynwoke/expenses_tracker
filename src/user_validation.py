...
def check_string_is_alphabetic(input: str) -> str:
    if not (re.match("^[a-zA-Z\s]+$", input) and len(input.strip()) !=0):
        raise ValueError
    return input

def check_input_is_numeric(input: str) -> float:
    return float(input)

def check_input_is_positive(input: str) -> float:
    if input <= 0:
        raise ValueError
    return input

def check_date_format(input: "date"):
